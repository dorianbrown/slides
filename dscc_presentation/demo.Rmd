...
conn <- dbConnect(SQLite(), '/home/dorian/data/FPA_FOD_20170508.sqlite')
...
fires %>%
    ...
    summarise(N = n()) %>%
    ggplot(aes(DISCOVERY_DATE, N)) + geom_line()
```

That didn't work. Lets look at some summary statistics
...
The max is pretty far off the median, lets have a look at that
...
Lets see how many NA dates we have

```{r}

fires %>%
    filter(DISCOVERY_DATE %>% is.na) %>%
    nrow()
...
fires %>%
    ...
    summarise(N = n()) %>%
    ggplot(aes(DISCOVERY_MTH, N)) + geom_line()
